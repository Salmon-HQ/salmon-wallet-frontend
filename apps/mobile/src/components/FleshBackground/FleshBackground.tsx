 ...
 * through the DOM. Both read `fleshTile` / `fleshFills` from `@salmon/shared`,
 * so neither platform owns the drawing and the two cannot drift apart. The
 * rationale for the geometry — and for why the tile wrap is baked into the
 * data rather than left to `overflow="visible"`, which `react-native-svg` has
 * no prop for — lives beside the data in `packages/shared/src/theme/flesh.ts`.
 *
