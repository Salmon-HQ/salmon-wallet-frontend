 ...
 * on the token detail sheet. Only the drawing differs, so only the drawing
 ...
 * - `descriptive` — true, useful on the detail sheet, and merely a category
 ...
 *
 * This is a deliberate port of the same table in `packages/ui`, not a shared
 * import: `apps/mobile` must not import `@salmon/ui`, which is DOM-only.
 * Keep the two in step when a tag is added.
 */
