...
| G9  | **A Powerup cannot fill `pending.kind`.** `TransactionProposal.pending.kind` is `'send' \| 'swap'` (`packages/shared/src/core/confirmation/types.ts:85`), and §3.6 requires the field. Stake's three transactions are neither, so the proposal does not typecheck.                                                                                                                                                                                                                               | Widen the union with the verbs a Powerup can take, and key the pending banner's copy off it — or let a Powerup pass its own summary with no kind. Core's call either way; the Powerup cannot solve it locally.                                                          |
| G10 | **Every receipt is an exchange.** `ConfirmationHost` hardcodes `ReceiptScreen tone="exchange"` on both twins, and `display.receipt` carries only `title`, `rate?`, `fee?`. A stake receipt cannot say when the stake activates — the one fact `AGENTS.md` step 2 says the user is owed.                                                                                                                                                                                                          | Let `display.receipt` carry a tone and rows, or give a Powerup the transfer receipt's `body`/`rows`. Until then a Powerup whose outcome is not a trade has a receipt that cannot describe it.                                                                           |
| G11 | **No block for a time-until-available line.** A cooling-down stake, a pending ramp order and a queued withdrawal all show a countdown that changes with no input. §1.7 covers waits that end in the same session; `PendingValue` is a "this number is being refetched" wrapper, not this.                                                                                                                                                                                                        | Add a `TimeRemaining`-style block on both twins, or rule that the line is a `KeyValueRow` whose value the Powerup recomputes on a timer of its own.                                                                                                                     |
| G12 | **One Powerup, several transactions.** The build route is one per id (`GET /v1/{networkId}/powerups/{id}/build`). Stake delegates, deactivates and withdraws. Nothing says whether that is one id with a build parameter, or something else.                                                                                                                                                                                                                                                     | Confirm one id plus a build parameter, and write it into §1.6 — three ids would mean three catalogue rows and three tabs for one capability.                                                                                                                            |
| G13 | **The core surface a Powerup implies has no step.** A stake account exists on chain whether or not the Powerup is installed, and the SOT requires the wallet to show it regardless, or turning a capability off would hide funds. That code is core, not `powerups/**` — the off build aliases the whole folder away — and §0 rule 1 ("logic lives once, in `powerups/<id>/`") points the wrong way for it.                                                                                      | Add a step to `AGENTS.md`: a position that exists without the Powerup is a core PR that lands first. Decide it once, because Stake is not the only capability with this shape.                                                                                          |
...
| G20 | **The manifest has no `kind`, but this document branches on one.** Three rules key off read-only versus transaction-building, including "direct third-party reads only for a read-only Powerup" (§2). A ramp does both: it builds the on-chain leg and must read the partner for order status, which the current wording forbids in combination.                                                                                                                                                 | Add `kind` to the manifest and say which rules follow from it, or rewrite §2's third-party rule so it is about what is read rather than about the Powerup's kind.                                                                                                       |
...
| G22 | **The extension mounts Powerup surfaces from a hardcoded chain of ids.** Mobile resolves generically through `getPowerupTab(id)`; the extension's Home branches on each id literally, with a component per branch. §2 calls both "registration in the three entries", which hides that one of the three is a hand-edited conditional inside a core screen — a community Powerup cannot land without editing it.                                                                                  | Give the DOM the same resolver mobile has, or write the asymmetry into §2 so nobody plans around a registry that is half a switch statement.                                                                                                                            |
