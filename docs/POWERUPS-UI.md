...
- **Mandatory:** the manifest fields `iconName`, `nameKey`, `descriptionKey`,
  ...
  `networks`, `tier`.
- **Fixed:** the tile (`ListRow` + `IconBubble` + the kit `Toggle`), the detail
  (row with `PowerupBadge`, the kit `Toggle`, About, What you can do, Uses,
...
- **Fixed:** where a Powerup is installed from. The `+` / `−` control is the
  same in the catalogue row and in the detail (owner, 2026-09-16): the row
  installs or removes in place, and tapping the rest of the row opens the
  detail, which is where the disclosure is read. A manifest cannot change
  either; a capability with custody exposure gets the same row and the same
  detail as every other.
