...
G9 onwards came from a **dry run of Stake against this document** on the same
day: an agent followed `AGENTS.md` §Starting a new Powerup and this runbook as
if implementing it, wrote no code, and reported where the documents failed it.
Every one was verified against the source before being written down. They are
