...
| G14 | ~~G7 assumed a Powerup's fee line could be exercised against the live swap build.~~ **Decided (owner, 2026-09-13):** Stake takes no Salmon fee. Salmon runs the validator it delegates to, so the revenue is that validator's commission, and a build fee on top would charge twice for one action in plain sight of a confirmation that shows the Salmon fee on its own line. The backend keeps nulling `salmonFee` on generic builds, and §1.6's fee line stays unexercised until a Powerup has one. | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| G15 | **A Powerup cannot declare an action-shaped surface.** `entries` carries only `tab?`, and both the id list Home is handed and the hook that builds its rows filter on it, so a Powerup with no tab is installed and mounted by nothing. Three of the eight planned capabilities are actions: the Ramp, Explore and Private Send.                                                                                                                                                                       | With the team, in Linear DEV-61, together with G16 — they are one decision. The owner leans to `entries.action` with the core deciding where it appears.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| G16 | **No slot on a core screen for a Powerup's control.** The balance block's contract fixes send, receive and activity; the token detail has no action row. Adding one is a core contract change on both twins that a Powerups-off build must alias away — the shape of G13.                                                                                                                                                                                                                              | The second half of G15, in Linear DEV-61.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| G17 | ~~**A Powerup that does not sign gets none of core's machinery.**~~ **Deferred (owner, 2026-09-13):** still true, and not a blocker. The Ramp is the only capability with this shape and the owner is building it end to end, backend and client together, so the decision is taken while building rather than before. The finding stands so whoever writes it knows what is missing before they start.                                                                                                | **Two shapes, and only one is safe — decided in advance because the unsafe one is the one that writes itself.** **Safe:** a separate type. A proposal that never carries bytes, never reaches the signer, and travels its own path to its own outcome screen; `requestSignature` stays the only door to bytes, and `TransactionProposal.transaction` stays required. **Unsafe:** making `transaction` optional on the proposal that already exists. Bytes and no-bytes then share one path, the signer's guard becomes conditional, and a conditional guard is one that is eventually evaluated wrong. The field being required is a protection, not a formality. Whichever way the surface is built, nothing under `powerups/**` gains an import it does not have today (`eslint.config.js`; `boundary.test.ts` asserts it fires). |
