 ...
 * centred on Send). Hoisted out of Send's amount step so any Powerup's
 * input form reuses the same visual (mobile
...
export interface AmountEntryCardPropsBase<TStyle> extends Testable {
  ...
  subtext?: string;
  /** True while the amount waits on something else (a quote). */
  loading?: boolean;
