...
describe('classifyBridgeError', () => {
  ...
  it('maps an above-maximum provider rejection to the maximum message', () => {
    ...
    expect(classifyBridgeError(new Error('Deposit amount is greater than maximum amount'))).toBe(
      'bridge.errors.aboveMaximum'
    );
  });
  ...
  it('maps an unreachable provider to the unavailable message', () => {
    ...
    expect(
      classifyBridgeError(new Error('Bridge create exchange failed: pair is not available'))
    ).toBe('bridge.errors.unavailable');
  });
