...
/** The drawing's native tile. Every scale is a multiple of it. */
const NATIVE_TILE = 26;
const TILE_WIDTH = 822;

/** The seigaiha tile: two rows of arcs, each split into halves. */
const SEIGAIHA_PATHS = [
  /** Row 1 — left half */
...
function tileUrl(stroke: string, strokeWidth: number, scale: number, tile: number): string {
  const svg =
    `<svg xmlns="http://www.w3.org/2000/svg" width="${TILE_WIDTH * scale}" height="${tile * scale}" viewBox="0 0 ${TILE_WIDTH} ${tile}">` +
    SEIGAIHA_PATHS.map(
