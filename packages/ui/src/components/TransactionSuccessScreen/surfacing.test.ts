 ...
 * it whole drags the Solana client into a test about milliseconds. The same
 * assertions guard the mobile copy in
 * `apps/mobile/src/components/TransactionSuccessScreen/surfacing.test.ts` —
 * two suites over the same numbers is what keeps the two copies one moment.
 */
import { describe, expect, it, vi } from 'vitest';

vi.mock('@salmon/shared', async () =>
  vi.importActual<Record<string, unknown>>('@salmon/shared/src/theme')
);
...
describe('the membrane alphas', () => {
  it('matches the alpha `surface.membraneThick` actually carries', () => {
    ...
    // value this module starts from. If the token is retuned, the element
    // opacity stops meaning α 0.55 and nothing else would notice.
