/**
 * The Surfacing — the timeline, as data. DOM port of
 * `apps/mobile/src/components/TransactionSuccessScreen/surfacing.ts`.
 *
 ...
 * *timing* can be tested without a renderer or a frame clock, exactly as the
 * mobile screen does it. The numbers are DESIGN.md's own and are kept local
 * for the same reason mobile keeps them local: `motionMs` covers the duration
 * ramp, but the band's geometry and the band→amount lag have no theme tokens,
 * and inventing shared tokens for one moment is not this component's call.
 * If either copy moves, move both — the mobile file is the reference.
 *
 ...
 * over the amount), and the amount still lands. What is removed is travel —
 * the success appears without the choreography.
 */
...
/**
 * The membrane is painted as one element filled with `surface.membraneThick`,
 * so "α 0.80 → 0.55" is an element opacity of 1 → 0.6875. Deriving the ratio
 * keeps the two halves of the statement from drifting apart.
 */
export const MEMBRANE_OPACITY_TO = MEMBRANE_ALPHA_TO / MEMBRANE_ALPHA_FROM;

/**
 * Height of the caustic band, in px. Its ink and its 0.5× scale live with the
 * rest of the motif, in `ScalesBackground`'s `caustic` variant.
 */
...
/** The amount rises this far, in px, as it settles. */
