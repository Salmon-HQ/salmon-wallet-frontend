...
Lot B does not touch `semantic.ts` (A owns it); it reads `surface.raised` + `withAlpha` directly until A's `surface.raisedFade` exists, then A replaces it. Lot C does not touch `semantic.ts` either; it reports whether `water.light` still has a consumer.
